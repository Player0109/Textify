...
CrisperWhisper 2.0 Large and Turbo FP16 entries; Accurate Canary-Qwen 2.5B;
...
### CrisperWhisper 2.0 / whisper.cpp Metal GPU

The catalog publishes two English-only Experimental FP16 entries through
Textify's pinned whisper.cpp/Metal integration:

| Model id | Exact file | Installed bytes | SHA-256 |
| --- | --- | ---: | --- |
| `crisperwhisper-2-large-f16` | `ggml-crisperwhisper-large-f16.bin` | 3,094,723,598 | `4160d90dcc11fb234bd126c85956e74c866b288524e8ec29a0a0a5c3e11c0cb7` |
| `crisperwhisper-2-turbo-f16` | `ggml-crisperwhisper-turbo-f16.bin` | 1,624,655,191 | `f9a12306a577c99cbdaab5a4988725fbceed5a92993baec10436e579116ffa41` |

Both conversions come from `drbaph/CrisperWhisper2.0-GGML` at immutable
revision `4807002cb64c57f78b37ffb4c48e986815a7c885`. Their original Nyra
checkpoints are `nyralabs/CrisperWhisper2.0_large` at
`fe94128aee7cdf1ad43ab59c4b1f7ed6362c1106` and
`nyralabs/CrisperWhisper2.0_turbo` at
`16424a0a0dad3d7bcc08927451718da49bce8ce0`. Textify uses its existing pinned
ggml-org whisper.cpp `v1.7.6` Metal integration at commit
`a8d002cfd879315632a579e73f0148d06959de36`; it does not ship or link the
CrisperWhisper.cpp runtime. Focused decoder logic in Textify's shim is adapted
from `Saganaki22/CrisperWhisper.cpp` commit
`13c7b3efdafd8bd20bd6361e7354ce2aa9bec464`, whose code is MIT-licensed.
Textify requires the verified Metal route, supports explicit English only, and
caps each runtime audio chunk at 30 seconds. The entries are not assigned
catalog benchmark ratings.

The standard public Nyra repositories publish noncommercial terms for the
model weights and outputs generated by them. Textify keeps the exact upstream
terms visible offline. Shipping these entries depends on Textify's separate
commercial grant from nyra health GmbH; the catalog and this guide do not
replace or modify the public upstream terms.
...
16 kHz mono English audio only, and caps each ASR window at 60 seconds within a
five-minute Dictation Session. Command-line
...
greedy decoding, the verified Metal GPU, and keeps every internal ASR window
within the model's 30-second limit.
...
and cap each ASR window at 60 seconds within a five-minute Dictation Session.
Automatic sends no language hint so the
