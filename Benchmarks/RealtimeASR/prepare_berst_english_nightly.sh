...
CORPUS="$SCRIPT_DIR/Corpus/berst-english-nightly-v1.json"
DATA_DIR="$SCRIPT_DIR/.benchmark-data/berst-english-nightly-v1"
TEMP_DIR="$(mktemp -d)"
...
while IFS=$'\t' read -r item_id config split row audio_path reference duration_ms \
  expected_size expected_sha speaker_id phone_position shout_level affect first_language \
  phone_model; do
  ...
  /usr/bin/jq -e \
    ...
    --arg phone_position "$phone_position" \
    --arg shout_level "$shout_level" \
    --arg affect "$affect" \
    --arg first_language "$first_language" \
    --arg phone_model "$phone_model" \
    '. as $entry
      | $entry.row_idx == $row
      and $entry.row.script == $reference
      and $entry.row.user_id == $speaker_id
      and $entry.row.phone_position == $phone_position
      and $entry.row.shout_level == $shout_level
      and $entry.row.affect == $affect
      and $entry.row.first_language == $first_language
      and $entry.row.phone_model == $phone_model
  ...
  /usr/bin/jq -r \
    '.items[] as $item
      ...
      | [$item.id, .config, .split, $item.row, $item.audio, $item.reference,
         ...
         $item.slices.phonePosition, $item.slices.shoutLevel, $item.slices.affect,
         $item.slices.firstLanguage, $item.slices.phoneModel]
      | @tsv' \
    "$CORPUS"
)

echo "Verified BERSt English nightly corpus at $DATA_DIR"
