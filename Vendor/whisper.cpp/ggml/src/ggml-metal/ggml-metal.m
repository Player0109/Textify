...
#define GGML_METAL_RELEASE(obj) do { (obj) = nil; } while (0)
#define GGML_METAL_RELEASE_ENUM(obj) do { (void)(obj); } while (0)
#define GGML_METAL_BLOCK_COPY(block) [block copy]
#define GGML_METAL_BLOCK_RELEASE(block) do { (block) = nil; } while (0)
#define GGML_METAL_DISPATCH_RELEASE(queue) do { (queue) = nil; } while (0)
#define GGML_METAL_OBJECT_PTR(obj) (__bridge void *)(obj)
#else
#define GGML_METAL_RELEASE(obj) [(obj) release]
#define GGML_METAL_RELEASE_ENUM(obj) [(obj) release]
#define GGML_METAL_BLOCK_COPY(block) Block_copy(block)
#define GGML_METAL_BLOCK_RELEASE(block) Block_release(block)
#define GGML_METAL_DISPATCH_RELEASE(queue) dispatch_release(queue)
#define GGML_METAL_OBJECT_PTR(obj) (void *)(obj)
#endif
...
#define GGML_METAL_ADD_KERNEL(e, name, supported) \
        if (supported) { \
            ...
            GGML_LOG_DEBUG("%s: loaded %-40s %16p | th_max = %4d | th_width = %4d\n", __func__, "kernel_"#name, GGML_METAL_OBJECT_PTR(kernel->pipeline), \
...
static void ggml_metal_free(struct ggml_backend_metal_context * ctx) {
    ...
    GGML_METAL_BLOCK_RELEASE(ctx->encode_async);
    ...
    GGML_METAL_DISPATCH_RELEASE(ctx->d_queue);
...
static void ggml_backend_metal_set_n_cb(ggml_backend_t backend, int n_cb) {
    ...
    ctx->encode_async = GGML_METAL_BLOCK_COPY(^(size_t iter) {
